...
export default function BlogIndexClient({ blogSection }: { blogSection: BlogContent }) {
  ...
  const posts: BlogPost[] = useMemo(() => getBlogListItems(blogSection), [blogSection]);
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {post.tags.map((tag) => (
                      <span
                        key={tag}
                        className="rounded-full bg-[#e5e7eb] px-3 py-1 text-xs font-semibold text-[#1f1f21]"
                      >
                        {tag}
                      </span>
                    ))}
